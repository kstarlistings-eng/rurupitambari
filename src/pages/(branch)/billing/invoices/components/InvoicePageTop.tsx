...
const INVOICE_PAGE_TEXT = {
  title: "Invoice Management",
  description: "Manage all billing invoices",
  mobileDescription: "Manage all billing invoices",
  addButton: "Create Invoice",
} as const;

function InvoicePageTop() {
  const navigate = useNavigate();
  return (
    <>
      <Page.HeaderLeftContainer className="min-[1170px]:w-auto w-full flex-wrap gap-y-2">
        <Page.HeaderContent>
          <Page.HeaderTitle>{INVOICE_PAGE_TEXT.title}</Page.HeaderTitle>
          <Page.HeaderDescription className="sm:block hidden">
            {INVOICE_PAGE_TEXT.description}
          </Page.HeaderDescription>
        </Page.HeaderContent>
        <div className="block min-[1170px]:hidden max-[360px]:ms-0 ms-auto">
          <Button variant={"default"} onClick={() => navigate("/sales-dispatch")}>
            {INVOICE_PAGE_TEXT.addButton}
          </Button>
        </div>
        <Page.HeaderDescription className="sm:hidden block w-full">
          {INVOICE_PAGE_TEXT.mobileDescription}
        </Page.HeaderDescription>
